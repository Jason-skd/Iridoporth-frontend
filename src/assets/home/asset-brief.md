...
- Backend API semantics from `../Iridoporth-backend`:
  ...
  - `GET/POST /api/v1/flight-log` means time-ordered logbook entries: id, content, callsign, created_at.
...
- The home page should not become the raspi status page or the flight-log page. It should introduce them narratively.
...
Iridoporth is a personal narrative site built around an aircraft window, a small machine signal, and a flight log.

The home page should feel like opening a travel notebook on a folded tray table: paper grain, stamped labels, ink marks, boarding-pass fragments, route lines, and a capsule-shaped aircraft window looking out over cloud layers and cabin light. The raspi status is an onboard instrument heartbeat. The flight log is a page waiting to be turned.
...
- More logbook than dashboard; more cabin window than landing page.
